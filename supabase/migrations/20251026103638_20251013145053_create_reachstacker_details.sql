/*
  # Create Reachstacker Details Table

  1. New Tables
    - `reachstacker_details`
      - `id` (uuid, primary key)
      - `dossier_id` (uuid, foreign key to dossiers)
      - All the same fields as empty_container_handler_details table
      - Fields for order information, equipment specs, drivetrain, mast, cabin, dimensions, and tires
      - Includes double_box and double_box_type fields

  2. Security
    - Enable RLS on `reachstacker_details` table
    - Add policies for authenticated users to read/write their own data
    - Manager and admin can access all data

  3. Notes
    - This table is identical in structure to empty_container_handler_details
    - Used specifically for reachstacker equipment type
*/

CREATE TABLE IF NOT EXISTS reachstacker_details (
  ...
  double_box boolean DEFAULT false,
  ...
  CONSTRAINT unique_dossier_reachstacker_details UNIQUE (dossier_id)
);

ALTER TABLE reachstacker_details ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Users can view reachstacker details they have access to"
  ON reachstacker_details FOR SELECT
  TO authenticated
  USING (
    EXISTS (
      SELECT 1 FROM dossiers
      WHERE dossiers.id = reachstacker_details.dossier_id
      AND (
...
CREATE POLICY "Users can insert reachstacker details for their dossiers"
  ON reachstacker_details FOR INSERT
  TO authenticated
  WITH CHECK (
    EXISTS (
      SELECT 1 FROM dossiers
...
CREATE POLICY "Users can update reachstacker details for their dossiers"
  ON reachstacker_details FOR UPDATE
  TO authenticated
  USING (
    EXISTS (
      SELECT 1 FROM dossiers
...
CREATE POLICY "Users can delete reachstacker details for their dossiers"
  ON reachstacker_details FOR DELETE
  TO authenticated
  USING (
    EXISTS (
      SELECT 1 FROM dossiers
...
CREATE INDEX IF NOT EXISTS idx_reachstacker_details_dossier_id ON reachstacker_details(dossier_id);