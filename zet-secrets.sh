...
ARGS=()
for KEY in FI_MACHINELIST_CODE FI_USERNAME FI_PASSWORD FI_EXPORT_MASCUS CRON_SECRET WC_URL WC_CONSUMER_KEY WC_CONSUMER_SECRET TRUCK1_PROVIDER_KEY TRUCK1_DEALER_ID TRUCK1_RELAY_URL TRUCK1_LOC_ID TRUCK1_CONTACT_PERSON_ID MASCUS_FEED_URL MASCUS_USERNAME MASCUS_PASSWORD; do
  VAL="${!KEY:-}"
  [ -n "$VAL" ] && ARGS+=("$KEY=$VAL")
done
